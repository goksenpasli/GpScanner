...
namespace MozJpeg
{
    ...
    public enum TJSubsamplingOptions
    {
        ...
        /// <summary>
        ///     4:2:2 chrominance subsampling. The JPEG or YUV image will contain one chrominance component for every 2x1 block of
        ///     pixels in the source image.
        /// </summary>
        TJSAMP_422,

        /// <summary>
        ///     4:2:0 chrominance subsampling. The JPEG or YUV image will contain one chrominance component for every 2x2 block of
        ///     pixels in the source image.
        /// </summary>
        TJSAMP_420,
    ...
    public class MozJpeg : IDisposable
    {
        ...
        /// <param name="flags">The bitwise OR of one or more of the <see cref="TJFlags" /> "flags"</param>
        /// <returns>Bitmap with image</returns>
        public void GetInfo(byte[] rawJpeg, out int width, out int height, out float horizontalResolution,
            out float verticalResolution, out TJSubsamplingOptions subsampl, out TJColorSpaces colorspace)
        {
        ...
        ///     <see cref="TJSubsamplingOptions" /> "Chrominance subsampling options".)
        /// </param>
            ...
            TJSubsamplingOptions subSamp = TJSubsamplingOptions.TJSAMP_420)
        {
            ...
            try
            {
                ...
                ulong bufSize = 0;
                if (UnsafeNativeMethods.TjCompress2(_compressorHandle, bmpData.Scan0, bmp.Width, bmpData.Stride,
                        bmp.Height, (int)tjPixelFormat, ref buf, ref bufSize, (int)subSamp, quality, (int)flags) ==
                    -1)
                {
    ...
    internal static class ClsArray
    {
    ...
    internal sealed class UnsafeNativeMethods
    {
        ...
        /// <param name="srcBuf">
        ///     Pointer to an image buffer containing RGB, grayscale, or CMYK pixels to be compressed. This buffer
        ///     is not modified.
        /// </param>
        ...
        ///         <item>
        ///             <description>
        ///                 pre-allocate the JPEG buffer with an arbitrary size using <see cref="TjAlloc" /> and let
        ...
        ///         <item>
        ///             <description>
        ///                 pre-allocate the buffer to a "worst case" size determined by calling <see cref="tjBufSize" />.
        ///                 This should ensure that the buffer never has to be re-allocated (setting
        ///                 <see cref="TJFlags.NOREALLOC" /> guarantees this.).
        ///             </description>
        ///         </item>
        ///     </list>
        ...
        public static int TjCompress2(IntPtr handle, IntPtr srcBuf, int width, int stride, int height, int pixelFormat,
            ref IntPtr jpegBuf, ref ulong jpegSize, int jpegSubsamp, int jpegQual, int flags)
        {
            switch (IntPtr.Size)
            {
                case 4:
        ...
        ///     where <c>scaledHeight</c> can be determined by calling <see cref="TJSCALED" /> with the JPEG image height and one
        ///     of the scaling factors returned by <see cref="tjGetScalingFactors" />.
        ...
        ///     <remarks>
        ///         Note: <c>scaledWidth</c> can be determined by calling <see cref="TJSCALED" /> with the JPEG image width and one
        ///         of the scaling factors returned by <see cref="tjGetScalingFactors" />
        ///     </remarks>
        ...
        ///     indicating the colorspace of the JPEG image(see <see cref="TJColorSpaces" /> "JPEG colorspaces".)
        /// </param>
        /// <returns>0 if successful, or -1 if an error occurred (see <see cref="tjGetErrorStr" />)</returns>
        public static int TjDecompressHeader(IntPtr handle, IntPtr jpegBuf, ulong jpegSize, out int width,
            out int height, out TJSubsamplingOptions jpegSubsamp, out TJColorSpaces jpegColorspace)
        {
            switch (IntPtr.Size)
            {
                case 4:
        ...
        ///     This function performs JPEG decompression but leaves out the color conversion step, so a planar YUV image is
        ///     generated instead of an RGB image.
        /// </summary>
        /// <param name="handle">A handle to a TurboJPEG decompressor or transformer instance</param>
        /// <param name="jpegBuf">Pointer to a buffer containing the JPEG image to decompress</param>
        /// <param name="jpegSize">Size of the JPEG image (in bytes)</param>
        /// <param name="dstPlanes">
        ///     An array of pointers to Y, U (Cb), and V (Cr) image planes (or just a Y plane, if decompressing a grayscale image)
        ///     that will receive
        ///     the YUV image.  These planes can be contiguous or non-contiguous in memory. Use #tjPlaneSizeYUV() to determine the
        ///     appropriate size for each plane based on
        ///     the scaled image width, scaled image height, strides, and level of chrominance subsampling.
        /// </param>
        /// <param name="width">
        ///     Desired width (in pixels) of the YUV image.  If this is different than the width of the JPEG image being
        ///     decompressed, then TurboJPEG will
        ///     use scaling in the JPEG decompressor to generate the largest possible image that will fit within the desired width.
        ///     If width is set to 0, then only the
        ///     height will be considered when determining the scaled image size.  If the scaled width is not an even multiple of
        ///     the MCU block width (see #tjMCUWidth), then
        ///     an intermediate buffer copy will be performed within TurboJPEG.
        /// </param>
        /// <param name="strides">
        ///     stride bytes per line in the image plane. Setting this to 0 is the equivalent of setting it to
        ///     the plane width.
        /// </param>
        /// <param name="height">
        ///     Desired height (in pixels) of the YUV image.  If this is different than the height of the JPEG image being
        ///     decompressed, then TurboJPEG
        ///     will use scaling in the JPEG decompressor to generate the largest possible image that will fit within the desired
        ///     height.  If height is set to 0, then only
        ///     the width will be considered when determining the scaled image size.  If the scaled height is not an even multiple
        ///     of the MCU block height (see #tjMCUHeight),
        ...
        ///     if an error occurred (see <see cref="tjGetErrorStr" />)
        /// </returns>
        ...
        /// <param name="width">
        ///     width (in pixels) of the YUV image.  NOTE: this is the width of the whole image, not the plane
        ///     width.
        /// </param>
        /// <param name="stride">
        ///     stride bytes per line in the image plane.  Setting this to 0 is the equivalent of setting it to
        ...
        /// </param>
        /// <param name="subsamp">
        ///     level of chrominance subsampling in the image (see @ref TJSAMP "Chrominance subsampling
        ///     options".)
        /// </param>
        /// <returns>
        ...
        private static extern int tjCompress2_x64(IntPtr handle, IntPtr srcBuf, int width, int pitch, int height,
        ...
        private static extern int tjCompress2_x86(IntPtr handle, IntPtr srcBuf, int width, int pitch, int height,
            int pixelFormat, ref IntPtr jpegBuf, ref ulong jpegSize, int jpegSubsamp, int jpegQual, int flags);
