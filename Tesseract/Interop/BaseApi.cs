...
namespace Tesseract.Interop
{
    ...
    public interface ITessApiSignatures
    {
        ...
        IntPtr BaseAPIGetComponentImages(HandleRef handle, PageIteratorLevel level, int text_only, IntPtr pixa,
            IntPtr blockids);
        ...
        int PageIteratorBoundingBox(HandleRef handle, PageIteratorLevel level, out int left, out int top, out int right,
            out int bottom);
        ...
        IntPtr PageIteratorGetImage(HandleRef handle, PageIteratorLevel level, int padding, HandleRef originalImage,
        ...
        void PageIteratorOrientation(HandleRef handle, out Orientation orientation,
            out WritingDirection writing_direction, out TextLineOrder textLineOrder, out float deskew_angle);
        ...
        IntPtr ResultIteratorWordFontAttributes(HandleRef handle, out bool isBold, out bool isItalic,
            out bool isUnderlined, out bool isMonospace, out bool isSerif, out bool isSmallCaps, out int pointSize,
            out int fontId);
        ...
        int TessBaseAPIDetectOrientationScript(HandleRef handle, out int orient_deg, out float orient_conf,
            out IntPtr script_name, out float script_conf);
    ...
    internal static class TessApi
    {
        ...
        {
            Guard.Require(nameof(handle), handle.Handle != IntPtr.Zero,
            ...
            foreach (KeyValuePair<string, object> pair in initialValues)
            {
                ...
                Guard.Require(nameof(initialValues), pair.Value != null,
                    "Variable '{0}': The type '{1}' is not supported.", pair.Key, pair.Value.GetType());
                varNames[i] = pair.Key;
        ...
        {
            Guard.Require(nameof(choiceIteratorHandle), choiceIteratorHandle.Handle != IntPtr.Zero,
