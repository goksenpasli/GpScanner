...
public static class PdfGeneration
{
    ...
    public static Task<PdfDocument> GeneratePdf(this List<ScannedImage> bitmapFrames, Format format, Paper paper,
            ...
            for (int i = 0; i < bitmapFrames.Count; i++)
            {
                ...
                else
                {
                    ...
                    BitmapSource resizedimage = format == Format.Tiff
                        ? scannedimage.Resim.BitmapSourceToBitmap().ConvertBlackAndWhite(Settings.Default.BwThreshold)
                            .ToBitmapImage(ImageFormat.Tiff).Resize(page.Width, page.Height, 0, dpi, dpi)
        ...
        List<ObservableCollection<OcrData>> ScannedText = null)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < imagefiles.Count; i++)
            {
                ...
                if (Scanner.PdfPageNumberDraw)
                {
                    gfx.DrawText(new XSolidBrush(XColor.FromKnownColor(Scanner.PdfAlignTextColor)), (i + 1).ToString(),
        ...
        ObservableCollection<OcrData> ScannedText = null)
    {
        ...
        try
        {
            ...
            if (Scanner.PdfPageNumberDraw)
            {
                gfx.DrawText(new XSolidBrush(XColor.FromKnownColor(Scanner.PdfAlignTextColor)), "1",
                    GetPdfTextLayout(page)[0], GetPdfTextLayout(page)[1]);
            }
        ...
        Format format, Paper paper, int jpegquality = 80, int dpi = 120)
    {
        ...
        try
        {
            ...
            {
                BitmapSource resizedimage = format == Format.Tiff
                    ? bitmapframe.BitmapSourceToBitmap().ConvertBlackAndWhite(Settings.Default.BwThreshold)
                        .ToBitmapImage(ImageFormat.Tiff).Resize(page.Width, page.Height, 0, dpi, dpi)
    ...
    {
        page.Size = paper == null || !paperSizes.TryGetValue(paper.PaperType, out PageSize pageSize)
            ? PageSize.A4
            : pageSize;
    }
    ...
    private static void DrawGfx(this XGraphics gfx, XBrush xBrush, XTextFormatter textformatter, OcrData item,
