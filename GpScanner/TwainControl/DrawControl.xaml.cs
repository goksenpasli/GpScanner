...
public partial class DrawControl : UserControl, INotifyPropertyChanged
{
    public DrawControl()
    {
        ...
        LoadImage = new RelayCommand<object>(parameter => TemporaryImage = EditingImage,
            parameter => EditingImage is not null);
    }
    ...
    public BitmapFrame EditingImage {
        get => (BitmapFrame)GetValue(EditingImageProperty);
        set => SetValue(EditingImageProperty, value);
    }
    ...
    private void Ink_PreviewMouseDown(object sender, MouseButtonEventArgs e)
    {
        if (e.RightButton == MouseButtonState.Pressed)
        {
            ...
            if (DrawControlContextMenu)
            {
                ...
                double widthmultiply = ((BitmapSource)Img.ImageSource).PixelWidth /
                                    (Ink.DesiredSize.Width < Ink.ActualWidth ? Ink.ActualWidth : Ink.DesiredSize.Width);
                double heightmultiply = ((BitmapSource)Img.ImageSource).PixelHeight /
                                     (Ink.DesiredSize.Height < Ink.ActualHeight
                                         ? Ink.ActualHeight
                                         : Ink.DesiredSize.Height);
