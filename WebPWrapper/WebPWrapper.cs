...
namespace WebPWrapper
{
    ...
    {
        public static bool WebpDllExists { get; } = Environment.Is64BitProcess
            ? File.Exists("libwebp_x64.dll")
            : File.Exists("libwebp_x86.dll");
        ...
        public Bitmap Decode(byte[] rawWebP)
        {
            ...
            try
            {
                GetInfo(rawWebP, out int imgWidth, out int imgHeight, out bool hasAlpha, out bool hasAnimation,
                    out string format);
                ...
                {
                    UnsafeNativeMethods.WebPDecodeBGRInto(ptrData, rawWebP.Length, bmpData.Scan0, outputSize,
                        bmpData.Stride);
                }
                else
                {
                    UnsafeNativeMethods.WebPDecodeBGRAInto(ptrData, rawWebP.Length, bmpData.Scan0, outputSize,
                        bmpData.Stride);
                }
        ...
        public byte[] EncodeLossless(Bitmap bmp)
        {
            ...
            {
                throw new NotSupportedException("Bitmap's dimension is too large. Max is " + WEBP_MAX_DIMENSION + "x" +
                                                WEBP_MAX_DIMENSION + " pixels.");
            }
            ...
            try
            {
                ...
                int size = bmp.PixelFormat == PixelFormat.Format24bppRgb
                    ? UnsafeNativeMethods.WebPEncodeLosslessBGR(bmpData.Scan0, bmp.Width, bmp.Height, bmpData.Stride,
                        out unmanagedData)
                    : UnsafeNativeMethods.WebPEncodeLosslessBGRA(bmpData.Scan0, bmp.Width, bmp.Height, bmpData.Stride,
                        out unmanagedData);
        ...
        public byte[] EncodeLossy(Bitmap bmp, int quality = 75)
        {
            ...
            try
            {
                ...
                size = bmp.PixelFormat == PixelFormat.Format24bppRgb
                    ? UnsafeNativeMethods.WebPEncodeBGR(bmpData.Scan0, bmp.Width, bmp.Height, bmpData.Stride, quality,
                        out unmanagedData)
                    : UnsafeNativeMethods.WebPEncodeBGRA(bmpData.Scan0, bmp.Width, bmp.Height, bmpData.Stride, quality,
                        out unmanagedData);
                if (size == 0)
                {
        ...
        /// <summary>
        ///     Compute PSNR, SSIM or LSIM distortion metric between two pictures. Warning: this function is rather CPU-
        ///     intensive
        /// </summary>
        ...
        public float[] GetPictureDistortion(Bitmap source, Bitmap reference, int metric_type)
        {
            ...
            try
            {
                ...
                if (sourceBmpData.PixelFormat == PixelFormat.Format32bppArgb)
                {
                    ...
                    if (UnsafeNativeMethods.WebPPictureImportBGRA(ref wpicSource, sourceBmpData.Scan0,
                            sourceBmpData.Stride) !=
                        1)
                    {
                ...
                {
                    wpicSource.use_argb = 1;
                    if (UnsafeNativeMethods.WebPPictureImportBGRA(ref wpicReference, referenceBmpData.Scan0,
                            referenceBmpData.Stride) !=
                        1)
                    {
                        throw new Exception("Can´t allocate memory in WebPPictureImportBGR");
                    }
                ...
                return UnsafeNativeMethods.WebPPictureDistortion(ref wpicSource, ref wpicReference, metric_type,
                    ptrResult) != 1
        ...
        private byte[] AdvancedEncode(Bitmap bmp, WebPConfig config, bool info)
        {
            ...
            try
            {
                ...
                if (bmp.Width > WEBP_MAX_DIMENSION || bmp.Height > WEBP_MAX_DIMENSION)
                {
                    throw new NotSupportedException("Bitmap's dimension is too large. Max is " + WEBP_MAX_DIMENSION +
                                                    "x" + WEBP_MAX_DIMENSION + " pixels.");
                }
    ...
    internal sealed class UnsafeNativeMethods
    {
        ...
        ///     'preset') and a given quality factor
        /// </summary>
        ...
        internal static void WebPDecodeARGBInto(IntPtr data, int data_size, IntPtr output_buffer,
            int output_buffer_size, int output_stride)
        {
            switch (IntPtr.Size)
            {
                case 4:
                    if (WebPDecodeARGBInto_x86(data, (UIntPtr)data_size, output_buffer, output_buffer_size,
                            output_stride) == default)
                    {
                ...
                case 8:
                    if (WebPDecodeARGBInto_x64(data, (UIntPtr)data_size, output_buffer, output_buffer_size,
                            output_stride) == default)
                    {
                        throw new InvalidOperationException("Can not decode WebP");
                    }
        ...
        /// <param name="output_buffer_size">Size of allocated buffer</param>
        /// <param name="output_stride">Specifies the distance between scan lines</param>
        internal static void WebPDecodeBGRAInto(IntPtr data, int data_size, IntPtr output_buffer,
            int output_buffer_size, int output_stride)
        {
            switch (IntPtr.Size)
            {
                case 4:
                    if (WebPDecodeBGRAInto_x86(data, (UIntPtr)data_size, output_buffer, output_buffer_size,
                            output_stride) == default)
                    {
                        throw new InvalidOperationException("Can not decode WebP");
                    }
                ...
                case 8:
                    if (WebPDecodeBGRAInto_x64(data, (UIntPtr)data_size, output_buffer, output_buffer_size,
                            output_stride) == default)
                    {
                        throw new InvalidOperationException("Can not decode WebP");
                    }
        ...
        /// <param name="output_buffer_size">Size of allocated buffer</param>
        /// <param name="output_stride">Specifies the distance between scan lines</param>
        internal static void WebPDecodeBGRInto(IntPtr data, int data_size, IntPtr output_buffer, int output_buffer_size,
            int output_stride)
        {
            switch (IntPtr.Size)
            {
                case 4:
                    if (WebPDecodeBGRInto_x86(data, (UIntPtr)data_size, output_buffer, output_buffer_size,
                            output_stride) == default)
                    {
                        throw new InvalidOperationException("Can not decode WebP");
                    }
                ...
                case 8:
                    if (WebPDecodeBGRInto_x64(data, (UIntPtr)data_size, output_buffer, output_buffer_size,
                            output_stride) == default)
                    {
                        throw new InvalidOperationException("Can not decode WebP");
                    }
        ...
        internal static int WebPEncodeBGR(IntPtr bgr, int width, int height, int stride, float quality_factor,
            out IntPtr output)
        {
            switch (IntPtr.Size)
            {
        ...
        /// <param name="output">output_buffer with WebP image</param>
        /// <returns>Size of WebP Image or 0 if an error occurred</returns>
        internal static int WebPEncodeBGRA(IntPtr bgra, int width, int height, int stride, float quality_factor,
            out IntPtr output)
        {
            switch (IntPtr.Size)
            {
        ...
        /// <summary>
        ///     Free any memory associated with the buffer. Must always be called last. Doesn't free the 'buffer' structure
        ///     itself
        /// </summary>
        ...
        /// <summary>
        ///     Validate the WebP image header and retrieve the image height and width. Pointers *width and *height can be
        ///     passed NULL if deemed irrelevant
        /// </summary>
        /// <param name="data">Pointer to WebP image data</param>
        /// <param name="data_size">This is the size of the memory block pointed to by data containing the image data</param>
        /// <param name="width">The range is limited currently from 1 to 16383</param>
        ...
        /// <summary>
        ///     Initialize the configuration as empty. This function must always be called first, unless WebPGetFeatures()
        ///     is to be called
        /// </summary>
        /// <param name="webPDecoderConfig">Configuration structure</param>
        ...
        internal static int WebPPictureDistortion(ref WebPPicture srcPicture, ref WebPPicture refPicture,
        ...
        /// <summary>
        ///     Release the memory allocated by WebPPictureAlloc() or WebPPictureImport*() Note that this function does
        ///     _not_ free the memory used by the 'picture' object itself. Besides memory (which is reclaimed) all other
        ///     fields of 'picture' are preserved
        /// </summary>
        ...
        private static extern IntPtr WebPDecodeARGBInto_x64([InAttribute] IntPtr data, UIntPtr data_size,
        ...
        private static extern IntPtr WebPDecodeARGBInto_x86([InAttribute] IntPtr data, UIntPtr data_size,
            IntPtr output_buffer, int output_buffer_size, int output_stride);

        [DllImport("libwebp_x64.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPDecodeBGRAInto")]
        private static extern IntPtr WebPDecodeBGRAInto_x64([InAttribute] IntPtr data, UIntPtr data_size,
            IntPtr output_buffer, int output_buffer_size, int output_stride);

        [DllImport("libwebp_x86.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPDecodeBGRAInto")]
        private static extern IntPtr WebPDecodeBGRAInto_x86([InAttribute] IntPtr data, UIntPtr data_size,
            IntPtr output_buffer, int output_buffer_size, int output_stride);

        [DllImport("libwebp_x64.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPDecodeBGRInto")]
        private static extern IntPtr WebPDecodeBGRInto_x64([InAttribute] IntPtr data, UIntPtr data_size,
            IntPtr output_buffer, int output_buffer_size, int output_stride);

        [DllImport("libwebp_x86.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPDecodeBGRInto")]
        private static extern IntPtr WebPDecodeBGRInto_x86([InAttribute] IntPtr data, UIntPtr data_size,
            IntPtr output_buffer, int output_buffer_size, int output_stride);
        ...
        private static extern int WebPEncodeBGR_x64([InAttribute] IntPtr bgr, int width, int height, int stride,
            float quality_factor, out IntPtr output);

        [DllImport("libwebp_x86.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPEncodeBGR")]
        private static extern int WebPEncodeBGR_x86([InAttribute] IntPtr bgr, int width, int height, int stride,
            float quality_factor, out IntPtr output);

        [DllImport("libwebp_x64.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPEncodeBGRA")]
        private static extern int WebPEncodeBGRA_x64([InAttribute] IntPtr bgra, int width, int height, int stride,
            float quality_factor, out IntPtr output);

        [DllImport("libwebp_x86.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPEncodeBGRA")]
        private static extern int WebPEncodeBGRA_x86([InAttribute] IntPtr bgra, int width, int height, int stride,
            float quality_factor, out IntPtr output);

        [DllImport("libwebp_x64.dll", CallingConvention = CallingConvention.Cdecl,
            EntryPoint = "WebPEncodeLosslessBGR")]
        private static extern int WebPEncodeLosslessBGR_x64([InAttribute] IntPtr bgr, int width, int height, int stride,
            out IntPtr output);

        [DllImport("libwebp_x86.dll", CallingConvention = CallingConvention.Cdecl,
            EntryPoint = "WebPEncodeLosslessBGR")]
        private static extern int WebPEncodeLosslessBGR_x86([InAttribute] IntPtr bgr, int width, int height, int stride,
            out IntPtr output);

        [DllImport("libwebp_x64.dll", CallingConvention = CallingConvention.Cdecl,
            EntryPoint = "WebPEncodeLosslessBGRA")]
        private static extern int WebPEncodeLosslessBGRA_x64([InAttribute] IntPtr bgra, int width, int height,
            int stride, out IntPtr output);

        [DllImport("libwebp_x86.dll", CallingConvention = CallingConvention.Cdecl,
            EntryPoint = "WebPEncodeLosslessBGRA")]
        private static extern int WebPEncodeLosslessBGRA_x86([InAttribute] IntPtr bgra, int width, int height,
            int stride, out IntPtr output);
        ...
        private static extern int WebPGetInfo_x64([InAttribute] IntPtr data, UIntPtr data_size, out int width,
            out int height);

        [DllImport("libwebp_x86.dll", CallingConvention = CallingConvention.Cdecl, EntryPoint = "WebPGetInfo")]
        private static extern int WebPGetInfo_x86([InAttribute] IntPtr data, UIntPtr data_size, out int width,
            out int height);
    ...
    internal enum WEBP_CSP_MODE
    {
        ...
        /// <summary>
        ///     Byte-order: RGB-565: [a4 a3 a2 a1 a0 r5 r4 r3], [r2 r1 r0 g4 g3 g2 g1 g0], ... WEBP_SWAP_16BITS_CSP is
        ///     defined,  Byte-order: RGB-565: [a4 a3 a2 a1 a0 b5 b4 b3], [b2 b1 b0 g4 g3 g2 g1 g0], ..
        /// </summary>
        MODE_RGBA_4444 = 5,

        /// <summary>
        ///     Byte-order: RGB-565: [r4 r3 r2 r1 r0 g5 g4 g3], [g2 g1 g0 b4 b3 b2 b1 b0], ... WEBP_SWAP_16BITS_CSP is
        ///     defined,  Byte-order: [b3 b2 b1 b0 a3 a2 a1 a0], [r3 r2 r1 r0 g3 g2 g1 g0], ..
        /// </summary>
        MODE_RGB_565 = 6,
    ...
    {
        /// <summary>
        ///     Main flag for encoder selecting between ARGB or YUV input. Recommended to use ARGB input (*argb,
